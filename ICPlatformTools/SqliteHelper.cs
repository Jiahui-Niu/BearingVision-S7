...
namespace ICPlatformTools
{
    ...
    public static class SQLiteConnectionManager
    {
        ...
        private static Dictionary<string, ConnectionInfo> m_connectons = new Dictionary<string, ConnectionInfo>();

        public static SQLiteConnection Create(string file)
        {
            var fullPath = Path.GetFullPath(file);
            lock (m_connectons)
            {
                if (m_connectons.ContainsKey(fullPath))
                {
                    var info = m_connectons[fullPath];
                    info.RefCount++;
                ...
                newConInfo.RefCount = 1;
                newConInfo.Connection = new SQLiteConnection(string.Format("data source={0}", file));
                m_connectons.Add(fullPath, newConInfo);
                newConInfo.Connection.Open();

                return newConInfo.Connection;
            }
        }
    ...
    public class SQLiteHelper
    {
        ...
        {
            var dt = GetTableList();
            var tableNames = new string[dt.Rows.Count];
            for (int i = 0; i < dt.Rows.Count; ++i)
            {
                tableNames[i] = dt.Rows[i]["Tables"] + "";
            }
            return tableNames;
        }
        ...
        {
            return new SQLiteCommand(m_connection);
        }

        public void CreateTable(SQLiteTableStructure table)
        {
        ...
        public void UpdateTableStructure(string targetTable, SQLiteTableStructure newStructure)
        {
