...
namespace ICPlatformTools
{
    ...
    public class KeyboardHook
    {
        ...
        * 模拟键盘输入
        * */
        public void Keyboard_Event(Keys key, KeyBoardEvent keyBoardEvent = KeyBoardEvent.Both)
        {
            Task.Run(() =>
            {
                ...
                case KeyBoardEvent.Both:
                    {
                        KeyboardHook.keybd_event((byte)(int)key, 0, 0, 0);
                        KeyboardHook.keybd_event((byte)(int)key, 0, 2, 0);
                    }
                    break;
        ...
        public void SendString(string str)
        {
            ...
            Task.Run(() =>
            {
                for (int i = 0; i < charArray.Length; i++)
                {
                    ...
                    keyInput[0].U.ki.wScan = (ScanCodeShort)charArray[i];
                    keyInput[0].U.ki.dwFlags = KEYEVENTF.UNICODE;
                    keyInput[0].U.ki.time = 0;

                    keyInput[1].type = INPUT_KEYBOARD;
                    keyInput[1].U.ki.wVk = 0;
                    keyInput[1].U.ki.wScan = (ScanCodeShort)charArray[i];
                    keyInput[1].U.ki.dwFlags = KEYEVENTF.UNICODE | KEYEVENTF.KEYUP;
                    keyInput[1].U.ki.time = 0;

                    while (SendInput(2, keyInput, Marshal.SizeOf(typeof(INPUT))) == 0)
                    {
                        Thread.Sleep(10);
                    }
                }
